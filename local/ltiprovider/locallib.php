...
function local_ltiprovider_populate($user, $context, $tool) {
    global $CFG;
    $custom_user_institution  = optional_param('custom_institution', null, PARAM_RAW);
    ...
    $user->institution = (!empty($custom_user_institution)) ? $custom_user_institution : "";
...
function local_ltiprovider_create_tool($courseid, $contextid, $lticontext) {
    ...
    $tool->timezone = 99;
    $tool->maildisplay = 2;
    $tool->city = ($lticontext->info['custom_city'] && !empty($lticontext->info['custom_city'])) ? $lticontext->info['custom_city'] : "mycity";
    $tool->country = ($lticontext->info['custom_country'] && !empty($lticontext->info['custom_country'])) ? $lticontext->info['custom_country'] : "ES";
    ...
    $tool->lastsync = 0;
    $tool->sendgrades = ($lticontext->info['custom_send_grades'] != null)
        ? $lticontext->info['custom_send_grades']
        : (!empty($lticontext->info['lis_outcome_service_url']))
            ? 1
            : 0;
