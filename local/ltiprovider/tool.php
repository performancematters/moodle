...
$custom_update_context          = optional_param('custom_update_context', false, PARAM_RAW);
$custom_user_specified_user_id  = optional_param('custom_user_specified_user_id', null, PARAM_RAW);
$custom_service 		= optional_param('custom_service', null, PARAM_RAW);
$send_grades 			= optional_param('custom_send_grades', null, PARAM_RAW);
...
if ($tool = $DB->get_record('local_ltiprovider', array('id'=>$toolid))) {
    if ($tool->sendgrades == 0 && $send_grades != null){
        $tool->sendgrades = 1;
        $DB->update_record('local_ltiprovider', $tool);
    } else if ($tool->sendgrades == 1 && $send_grades == null){
        $tool->sendgrades = 0;
	$DB->update_record('local_ltiprovider', $tool);
    }
    if ($tool->disabled) {
...
{
    $month = optional_param('custom_course_start_month', 0, PARAM_RAW);
    $day = optional_param('custom_course_start_day', 0, PARAM_RAW);
    $year = optional_param('custom_course_start_year', 0, PARAM_RAW);
    return mktime(0, 0, 0, $month, $day, $year);
}

function update_existing_course($context, $course, $DB)
{

    $course->fullname = local_ltiprovider_get_new_course_info('fullname', $context);
    $course->shortname = local_ltiprovider_get_new_course_info('shortname', $context);
    $course->format = optional_param('custom_course_format', null, PARAM_RAW);
    $course->cdesc = optional_param('custom_description', null, PARAM_RAW);
    $course->numsections = optional_param('custom_numsections', null, PARAM_RAW);
    $course->startdate = get_course_start_date();
...
function create_new_course($context, $DB, $custom_user_specified_user_id)
{
    ...
    $newcourse->format = optional_param('custom_course_format', null, PARAM_RAW);
    $newcourse->cdesc = optional_param('custom_description', null, PARAM_RAW);
    $newcourse->numsections = optional_param('custom_numsections', null, PARAM_RAW);
    $newcourse->startdate = get_course_start_date();
    ...
    if ($custom_context_template and $tplcourse) {
        ...
        $username = $custom_user_specified_user_id != null
            ? $custom_user_specified_user_id
            : local_ltiprovider_create_username($context->info['oauth_consumer_key'], $context->info['user_id']);
...
if ($context->valid) {
    ...
    $idnumber = local_ltiprovider_get_new_course_info('idnumber', $context);

    $course = $DB->get_record('course', array('idnumber' => $idnumber));

    if ($custom_service != null && $custom_service == 'delete_course')
    {
    ...
    if ($custom_create_context) {
        ...
        $tool = create_new_course($context, $DB, $custom_user_specified_user_id);
    } else if ($custom_update_context){
        $tool = update_existing_course($context, $course, $DB);
    }
    ...
    if ($custom_service != null && $custom_service == 'unenroll_user')
    {
        $tool = $DB->get_record('local_ltiprovider', array('contextid' => $coursecontext->id));
        local_ltiprovider_unenrol_user($tool, $dbuser);
        return;
    }
