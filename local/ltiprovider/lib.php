                        ...
                        if ($context = $DB->get_record('context', array('id' => $tool->contextid))) {
                            if ($context->contextlevel == CONTEXT_COURSE) {
                                ...
                                if ($tool->requirecompletion and !$completion->is_course_complete($user->userid)) {
                                    ...
                                    continue;
                                } else if ($tool->requirecompletion and $completion->is_course_complete($user->userid)){
                                    mtrace("  User is complete $user->userid for course $tool->courseid");
                                    $user_complete = 1;
                                }
                            ...
                            if ( $grade === false || $grade === NULL || strlen($grade) < 1) {
                                mtrace( " the user complete $user_complete");
                                //if the grade is not set, we will assume this is completion by
                                //activity only, and the course does not need a grade.
                                //set it to 100 so empari gets complete back
                                if ($user_complete){
                                    $grade = 100;
                                } else {
