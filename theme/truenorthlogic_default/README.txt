...
This document describes how to copy and customise the truenorthlogic_default (bootstrapbase) theme so that
...
This theme was copied and modified from the Clean theme. The Clean theme can be re-installed by changing the
Clean theme folder name from "clean theme" to "clean".

Getting started
---------------

From your Moodle theme directory right click on truenorthlogic_default and then copy and paste back
into your Moodle theme directory. You should now have a folder called Copy of truenorthlogic_default.
...
underscores. For the purpose of this tutorial we will call the theme 'newname'.

On opening 'truenorthlogic_default' your you will find several files and sub-directories which have
files within them.
...
where the old theme name occurs, in this case tnlbasic. So using the above list as
a guide, search through and change all the instances of the theme name
'truenorthlogic_default' to 'newname'. This includes the filename of the lang/en/truenorthlogic_default.php.
You need to change this to 'theme_newname.php'.
...
and then click on (newname) or whatever you renamed your theme to,
from the list of theme names that appear at this point in the side block.

Customisation using custom theme settings
-----------------------------------------

The settings page for the truenorthlogic_default theme can be located by navigating to:

Administration > Site Administration > Appearance > Themes > truenorthlogic_default

Moodle documentation
--------------------

Further information can be found on Moodle Docs: http://docs.moodle.org/dev/
